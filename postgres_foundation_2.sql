...
-- Regex cast insensative
SELECT * FROM film WHERE title ~* 'iron';

-- POSIX + Perl style regex patterns
SELECT * FROM film WHERE title ~* '^iron';
...
SELECT to_char(lower(rental_period), 'Day, Month DD, YYYY'), count(*)
FROM rental
WHERE rental_period <@ tstzrange('2023-07-01'::timestamptz,NULL)
GROUP BY 1
ORDER BY 2 DESC;
 ...
 */
UPDATE customer SET activebool = FALSE WHERE customer_id IN
	(SELECT customer_id FROM customer TABLESAMPLE bernoulli(0.02));
...
-- UPDATE customer SET activebool=true WHERE activebool=false;
...
SELECT inventory_id, array_agg(customer_id) rental_dates FROM rental
GROUP BY inventory_id 
ORDER BY inventory_id;

SELECT inventory_id, array_agg(customer_id ORDER BY lower(rental_period) DESC) rental_dates FROM rental
GROUP BY inventory_id 
ORDER BY inventory_id;

-- Use this in conjunction with other array operators!
SELECT inventory_id, array_to_string(array_agg(customer_id),',') rental_dates FROM rental
GROUP BY inventory_id 
ORDER BY inventory_id;
...
-- title.
SELECT film_id, g.* FROM film CROSS JOIN LATERAL 
    UNNEST(string_to_array(title, ' ')) WITH ORDINALITY AS g(w,o);
